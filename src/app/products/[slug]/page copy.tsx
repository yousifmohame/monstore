...
import { Star, ShoppingCart, Heart, Share2, Minus, Plus, Truck, Shield, RotateCcw, MessageCircle, Eye, Play, X, Palette, Ruler } from 'lucide-react';
...
import { mockProducts, colorOptions, sizeOptions } from '@/lib/mockData';
...
export default function ProductDetailPage({ params }: { params: { slug: string } }) {
  ...
  const loadProduct = async () => {
    try {
      ...
      if (foundProduct) {
        ...
        // Set default color and size if product has variants
        if (foundProduct.hasVariants) {
          if (foundProduct.colors && foundProduct.colors.length > 0) {
            setSelectedColorId(foundProduct.colors[0]);
          }
          
          if (foundProduct.sizes && foundProduct.sizes.length > 0) {
            setSelectedSizeId(foundProduct.sizes[0]);
          }
        }
        ...
        setReviews([
          {
            ...
            rating: 5,
            title: 'منتج رائع!',
            comment: 'جودة ممتازة وتصميم جميل. أنصح بشرائه بشدة. وصل في الوقت المحدد والتعبئة كانت ممتازة.',
  ...
  useEffect(() => {
    ...
    if (product?.hasVariants && product.variants) {
      const variant = product.variants.find((v: any) => 
        (v.colorId === selectedColorId || (!v.colorId && !selectedColorId)) && 
        (v.sizeId === selectedSizeId || (!v.sizeId && !selectedSizeId))
      );
  ...
  const handleAddToCart = () => {
    // Check if product has variants and if a variant is selected
    ...
    const colorName = selectedColorId ? colorOptions.find(c => c.id === selectedColorId)?.nameAr : null;
    const sizeName = selectedSizeId ? sizeOptions.find(s => s.id === selectedSizeId)?.nameAr : null;

    addToCart({
      id: selectedVariant ? `${product.id}-${selectedVariant.id || 'default'}` : product.id,
      name: product.nameAr,
  ...
  const handleShare = async () => {
    try {
      // Use clipboard API as a fallback since Web Share API may not be available in all contexts
  ...
  const handleSubmitReview = (e: React.FormEvent) => {
    ...
    const review: Review = {
      ...
      verifiedPurchase: true, // Mock verified purchase
                  ...
                  <h3 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
                    ...
                    {selectedColorId && (
                      <span className="font-normal text-gray-600">
                        {colorOptions.find(c => c.id === selectedColorId)?.nameAr}
                      </span>
                    )}
                  </h3>
              ...
              {product.hasVariants && product.sizes && product.sizes.length > 0 && (
                <div className="mb-6">
                  <h3 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
                    ...
                    {selectedSizeId && (
                      <span className="font-normal text-gray-600">
                        {sizeOptions.find(s => s.id === selectedSizeId)?.nameAr}
                      </span>
                    )}
                  </h3>
                  <div className="flex flex-wrap gap-3">
                        ...
                        <div className="flex justify-between py-2 border-b border-gray-200">
                          ...
                          <span className="text-gray-600">
                            {product.colors?.length > 0 
                              ? product.colors.map((colorId: string) => 
                                  colorOptions.find(c => c.id === colorId)?.nameAr
                                ).join('، ')
                              : 'لا يوجد'
                            }
                          </span>
                        ...
                        <div className="flex justify-between py-2 border-b border-gray-200">
                          ...
                          <span className="text-gray-600">
                            {product.sizes?.length > 0 
                              ? product.sizes.map((sizeId: string) => 
                                  sizeOptions.find(s => s.id === sizeId)?.nameAr
                                ).join('، ')
                              : 'لا يوجد'
                            }
                          </span>
